...
class Mt_MobCntIconManager: public QObject
{
Q_OBJECT

public:
    Mt_MobCntIconManager();
	~Mt_MobCntIconManager();
...
Mt_MobCntIconManager::Mt_MobCntIconManager()
    {
    }
...
void Mt_MobCntIconManager::initTestCase()
    {
    mIconMgr = new MobCntIconManager();
    
    }

void Mt_MobCntIconManager::cleanupTestCase()
    {
    delete mIconMgr;
    mIconMgr = NULL;
    }

void Mt_MobCntIconManager::init()
    {
    initTestCase();
    }
     
void Mt_MobCntIconManager::cleanup()
    {
    cleanupTestCase();
    }

void Mt_MobCntIconManager::testNonExistingIcon()
    {
...
void Mt_MobCntIconManager::testOneExistingIcon()
    {
    QSignalSpy spy(mIconMgr, SIGNAL(contactIconReady(int)));
    Q_ASSERT(spy.isValid());
    QCOMPARE( spy.count(), 0 );
...
void Mt_MobCntIconManager::testCancel()
    {
    QSignalSpy spy(mIconMgr, SIGNAL(contactIconReady(int)));
    Q_ASSERT(spy.isValid());
    QCOMPARE( spy.count(), 0 );
...
void Mt_MobCntIconManager::testMultipleExistingIcon()
    {
    QSignalSpy spy(mIconMgr, SIGNAL(contactIconReady(int)));
    Q_ASSERT(spy.isValid());
    QCOMPARE( spy.count(), 0 );
...
QTEST_MAIN(Mt_MobCntIconManager)
#include "mt_mobcnticonmanager.moc"
